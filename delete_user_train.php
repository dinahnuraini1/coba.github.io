...
if (isset($_GET['kd_user'])) {
    ...
    if ($con->query($query) === TRUE) {
        ...
        header("Location: user_train_checker.php?delete_success=true");
        exit();
    } else {
        // Redirect kembali ke halaman sebelumnya dengan pesan error jika query gagal
        header("Location: user_train_checker.php?delete_error=true");
        exit();
    }
} else {
    // Redirect kembali ke halaman sebelumnya jika parameter kd_user tidak diterima
    header("Location: user_train_checker.php");
    exit();
}
