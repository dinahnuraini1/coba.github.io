...
// Contoh pemanggilan fungsi log_activity
$kd_user = 2; // ID admin yang melakukan operasi CRUD
log_activity($kd_user, "Mengedit entri"); // Log untuk operasi Update
log_activity($kd_user, "Menghapus entri"); // Log untuk operasi Delete
?>
...
if (isset($_GET['kd_user'])) {
    ...
    $data_user = mysqli_query($con1, $query);
    ...
    if ($data_user) {
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($old_password != $q_user['password']) {
                echo "
                    <script>
                        ...
                        window.location.href = 'update_user_tayangandaop7.php?kd_user=$kd_user';
                    </script>
                ";
                exit();
            }
            ...
            if ($new_password != $konf_password) {
                echo "
                    <script>
                        ...
                        window.location.href = 'update_user_tayangandaop7?kd_user=$kd_user';
                    </script>
                ";
                exit();
            }
            ...
            $update_query = "UPDATE tc_tb_user SET nm_user='$nama_user', unit='$nama_unit', kd_level='$kd_level', password='$konf_password' WHERE kd_user='$kd_user'";
            $result = mysqli_query($con1, $update_query);

            if ($result) {
                echo "
                    <script>
                        alert('Data Berhasil Diubah');
                        window.location.href = 'user_tayangandaop7.php';
                    </script>
                ";
                exit();
            } else {
                echo "Gagal mengupdate data: " . mysqli_error($con1);
            }
        }
    } else {
        echo "Query error: " . mysqli_error($con1);
    }
}
?>
...
<head>
    ...
    <title>Update- User Tayangan DAOP 7</title>
                            ...
                            <div class="p-3 pb-4 pt-4">
                                ...
                                <form method="POST">
                                    <div class=" mb-3 mt-3">
                                        ...
                                        <input type="text" name="nama_unit" class="form-control" value="<?php echo $q_user['unit'] ?>" />
                                        <br>
                                        <label>Level</label>
                                    ...
                                    <a <button href="user_tayangandaop7.php" type="submit" class="btn btn-danger">Kembali</button> </a>
